...
│  │                                                         │    │
│  │  • AssetIndex      (Stores: Weather API asset)         │    │
│  │  • PolicyStore     (Stores: allow-all policy)          │    │
│  │  • ContractStore   (Stores: weather-contract-def)      │    │
│  └────────────────────────────────────────────────────────┘    │
│                                                                  │
│  ┌────────────────────────────────────────────────────────┐    │
...
```json
{
  "id": "allow-all-policy",
  "policy": {
    "permissions": [],    // Empty = no restrictions
    "prohibitions": [],   // Empty = nothing forbidden
    "obligations": []     // Empty = no requirements
  }
}
```

### Contract Definition: "weather-contract-def"
```json
{
  "id": "weather-contract-def",
  "accessPolicyId": "allow-all-policy",      // Who can see this
  "contractPolicyId": "allow-all-policy",    // Rules for usage
  "assetsSelector": {
    "operandLeft": "id",
    "operator": "=",
...
(This PoC)                           (Another instance)
     ...
     │  2. Catalog Response                 │
     │──────────────────────────────────────►│
     │     Returns: weather-contract-def    │
     │                                      │
     │  3. Contract Negotiation Request     │
     │◄─────────────────────────────────────│
     │     POST /api/dsp/negotiations       │
     │     Body: {                          │
     │       offerId: "weather-contract..", │
     │       assetId: "weather-api-asset"   │
     │     }                                │
     │                                      │
     │  4. Provider validates policy        │
     │     - Checks "allow-all-policy"      │
     │     - Always passes ✅                │
     │                                      │
...
Even though our policy is "allow-all", here's how evaluation works:
